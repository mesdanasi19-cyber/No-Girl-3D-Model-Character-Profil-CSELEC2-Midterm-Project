...
if (!gl.getProgramParameter(helloTriangleProgram, gl.LINK_STATUS)) {
  ...
  // =========================================================
  // 1. UI TOGGLE & 3D TEXT TRANSITION VARIABLES
  // =========================================================
  const toggleBtn = document.getElementById('toggle-profile-btn');
  const appContainer = document.getElementById('app-container');

  let isExpanded = false;
  let textOffsetX = 0.0;        // Current animated position of 3D text
  let targetTextOffsetX = 0.0;  // Target position when toggled

  if (toggleBtn && appContainer) {
    toggleBtn.addEventListener('click', () => {
      isExpanded = !isExpanded;
      appContainer.classList.toggle('expanded', isExpanded);
      // Shift 3D text right in WebGL space when expanded
      targetTextOffsetX = isExpanded ? 2.1 : 0.0;
    });
  }
  ...
  function render(now) {
    ...
    // Smooth linear interpolation (lerp) for 3D text sliding movement
    textOffsetX += (targetTextOffsetX - textOffsetX) * 0.08;

    gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
    gl.useProgram(helloTriangleProgram);

    // =========================================================
    // 1. DRAW "NO-GIRL" TEXT (Uses full 800px canvas viewport)
    // =========================================================
    gl.viewport(0, 0, canvas.width, canvas.height); // Full viewport for text sliding

    const dynamicAspect = canvas.width / canvas.height;
    const textProjectionMatrix = mat4Perspective(Math.PI / 4, dynamicAspect, 0.1, 100);

    const fixedEye = [0 - textOffsetX, 0, 5.0];
    const fixedTarget = [0 - textOffsetX, 0, 0];
    const fixedViewMatrix = mat4LookAt(fixedEye, fixedTarget, [0, 1, 0]);
    const fixedViewProjection = mat4Multiply(textProjectionMatrix, fixedViewMatrix);
    ...
    // =========================================================
    // 2. DRAW MAIN 3D OBJECT (Fixed to left 400px region)
    // =========================================================
    // Lock viewport strictly to left 400px (or displayHeight x displayHeight area)
    const fixedViewportWidth = displayHeight; // 400px
    gl.viewport(0, 0, fixedViewportWidth, displayHeight);

    // Fixed 1.0 aspect ratio (400 / 400) keeps the brain perfectly proportional
    const fixedAspect = 1.0; 
    const projectionMatrix = mat4Perspective(Math.PI / 4, fixedAspect, 0.1, 100);

    // Calculate normal camera position around center [0, 0, 0]
    ...
    // Update viewProjection uniform for 3D model
    gl.uniformMatrix4fv(uViewProjectionLocation, false, viewProjectionMatrix);
    gl.uniform3fv(uViewPosLocation, eye);
